...
from selectabletest.models import Thing, ThingLookup
...
class AutoCompleteSelectMultipleWidgetTestCase(BaseSelectableTestCase, WidgetTestMixin):
    ...
    def test_render_single(self):
        ...
        self.assertEqual(field.attrs['data-selectable-type'], 'hidden-multiple')
        self.assertEqual(field.attrs['type'], 'hidden')
        self.assertEqual(int(field.attrs['value']), val)
    ...
    def test_render_list(self):
        ...
        for field in inputs['field_name_1']:
            self.assertEqual(field.attrs['data-selectable-type'], 'hidden-multiple')
            self.assertEqual(field.attrs['type'], 'hidden')
            found_values.append(int(field.attrs['value']))
...
class AutoComboboxSelectMultipleWidgetTestCase(BaseSelectableTestCase, WidgetTestMixin):
    ...
    def test_render_single(self):
        ...
        self.assertEqual(field.attrs['data-selectable-type'], 'hidden-multiple')
        self.assertEqual(field.attrs['type'], 'hidden')
        self.assertEqual(field.attrs['value'], str(val))
